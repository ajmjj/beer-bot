...
console.log(`${nullPush.length} beers with NULL push_name: ${fixable.length} resolvable, ${missing.length} unmatched.`);
...
for (const r of fixable) {
  ...
  if (!APPLY) { console.log(`  would set #${r.beer_number} push_name=${JSON.stringify(r.name)}${placeholder ? ` member=${JSON.stringify(r.name)}` : ` (keep member ${JSON.stringify(r.member)})`}`); continue; }
  const { error } = await supabase.from("beers").update(patch).eq("beer_number", r.beer_number);
  if (error) { console.error(`  #${r.beer_number} failed: ${error.message}`); continue; }
  updated++;
}
...
console.log(APPLY ? `Applied: updated ${updated} beers. Wrote missing-push-names.md (${missing.length}).` : `Dry run. Wrote missing-push-names.md (${missing.length}). Re-run with --apply to update.`);
