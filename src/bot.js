...
import { insertBeers, markBeerDeleted, getMemberName, handleBeerEdit, syncMembers, updateMemberPushName, claimBackfillBeers } from "./store.js";
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        if (inserted && msg.pushName) {
          const part = num(msg.key.participant);
          updateMemberPushName(part, msg.pushName).catch(() => {});
          claimBackfillBeers(part, msg.pushName)
            .then((n) => n && console.log(`claimed ${n} backfill beer(s) for ${msg.pushName}`))
            .catch(() => {});
        }
      } catch (err) {
